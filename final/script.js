...
let currentColor = color.toHEXA().toString();

let colorWhite = "#fff";
let colorBlack = "#000";

checkColors(colorWhite,currentColor)
let contrastWhite = colorData.contrast;


checkColors(colorBlack,currentColor)
let contrastBlack = colorData.contrast;

let hslaColor = color.toHSLA().toString();
console.log(hslaColor)
let hslColor = hslaColor.replace(/[a ]/g, '').slice(0,-3)+ ')'

var regex = /hsl\([0-9]+,[.0-9]+%,([.0-9]+%)\)/;

var hslResult = hslColor.match(regex);



console.log(hslColor);

if(contrastBlack < 1.2 || contrastWhite < 1.2 || parseFloat(hslResult[1]) > 65 || parseFloat(hslResult[1] < 15)){
alert("Contrast is too low!!!")
}else{
  let currentDarkerColor = hslResult[0].replace(hslResult[1], parseInt(hslResult[1]) > 40 ? parseInt(hslResult[1]) - 40 + '%' : '15%')
  
  document.documentElement.style.setProperty('--main-skin-color-900',currentColor);
  document.documentElement.style.setProperty('--main-skin-color-100',currentDarkerColor);
